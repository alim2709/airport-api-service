...
def sample_flight(**params):
    ...
    defaults = {
        "route": route,
        "airplane": airplane,
        "departure_time": "2023-07-10",
        "arrival_time": "2023-09-11",
    }
    defaults.update(params)
    ...
    def test_create_flight(self):
        with patch("service.models.datetime") as now_time_mock:
            ...
            payload = {
                ...
                "departure_time": datetime.datetime(2023, 7, 11, 0, 0),
                "arrival_time": datetime.datetime(2023, 9, 11, 0, 0),
    ...
    def test_update_flight(self):
        with patch("service.models.datetime") as now_time_mock:
            ...
            payload = {
                "route": route.id,
                "airplane": airplane.id,
                "departure_time": datetime.datetime(2023, 7, 12, 0, 0),
                "arrival_time": datetime.datetime(2023, 9, 13, 0, 0),
                "crew": [crew1.id, crew2.id]
            }
            res = self.client.put(url, payload)
            self.assertEquals(res.status_code, status.HTTP_200_OK)
